...
def test_hierarchy_invoker_too_low(create_mock_member):
    ...
    assert (
        error
        == "You cannot moderate a member with an equal or higher role than yourself."
    )


def test_hierarchy_bot_too_low(create_mock_member):
    bot = create_mock_member(
        999, top_role_position=30, is_bot=True
    )  # Bot has lower role than target
    ...
    assert (
        error
        == "I cannot moderate a member with an equal or higher role than my highest role."
    )
